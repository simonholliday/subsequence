...
- **Times are in beats; steps count grid slots.** `beat=`, `spacing=`, and `duration=` are in beats; `hit_steps`, `sequence`, and the decorator's `steps=` count grid steps. A negative `beat=` counts back from the end of the pattern, in every verb that places something and in `capture()`, which reads from there: `beat=-1` is the last beat.
