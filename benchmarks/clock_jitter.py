    ...
    async def _run() -> None:
        ...
        try:
            await asyncio.wait_for(
                asyncio.shield(seq.task), timeout=total_seconds + 2.0
...
def main() -> None:
    ...
    parser.add_argument(
        "--bpm", type=float, default=120, help="Tempo in BPM (default: 120)"
    )
    parser.add_argument(
        "--bars", type=int, default=32, help="Bars to measure (default: 32)"
    )
    parser.add_argument(
        "--no-spin-wait",
        action="store_true",
        help="Disable spin-wait (use pure asyncio.sleep)",
    )
    parser.add_argument(
        "--device", type=str, default=None, help="MIDI output device name"
    )
    parser.add_argument(
    ...
    if args.compare:
        ...
        spin_jitter = _run_benchmark(
            args.bpm, args.bars, spin_wait=True, device_name=args.device
        )
        _print_report(
            spin_jitter, args.bpm, args.bars, spin_wait=True, label="[spin-wait ON]"
        )

        print("Running with spin-wait OFF ...")
        pure_jitter = _run_benchmark(
            args.bpm, args.bars, spin_wait=False, device_name=args.device
        )
        _print_report(
            pure_jitter, args.bpm, args.bars, spin_wait=False, label="[spin-wait OFF]"
        )

    else:
        spin = not args.no_spin_wait
        jitter = _run_benchmark(
            args.bpm, args.bars, spin_wait=spin, device_name=args.device
        )
