...
def drums(p):
    ...
    if section == "void":
        ...
        p.cellular_1d(
            "kick_1", rule=30, velocity=28, no_overlap=True, probability=ca_probability
        )
    ...
    # ═══════════════════════════════════════════════════════════════
        ...
        if t >= 0.5:
            ...
            p.bresenham_poly(
                ...
                velocity={"hi_hat_closed": 50 + round(14 * hat_feel)},
            )

        # Transition: snare roll if heading to swarm
        if p.section.last_bar and p.section.next_section == "swarm":
            p.hit_steps(
                "snare_1", [8, 10, 12, 13, 14, 15], velocity=round(50 + 30 * ease(t))
            )
    ...
    # ═══════════════════════════════════════════════════════════════
        ...
        p.cellular_1d(
            "kick_1", rule=30, velocity=34, no_overlap=True, probability=ca_probability
        )

        # Kick ghost fill: sixteenths bias, enters at 0.15
        if t >= 0.15:
            gf_d = (0.05 + 0.15 * ease(min(1.0, (t - 0.15) / 0.85))) * (
                0.5 + ghost_wander * 0.5
            )
            if gf_d > 0.02:
                p.ghost_fill(
                    "kick_1",
        ...
        if p.section.last_bar and p.section.next_section == "fury":
    ...
    # ═══════════════════════════════════════════════════════════════
        ...
        p.ghost_fill(
            "snare_1", density=gs_d, velocity=(22, 42), bias="before", no_overlap=True
        )
        ...
        p.cellular_1d(
            "hi_hat_closed", rule=90, velocity=42, no_overlap=True, probability=0.64
        )
        ...
        if chaos_spark > 0.6:
            ...
            p.ghost_fill(
                ...
                velocity=(spark_vel - 12, spark_vel),
                bias="offbeat",
                no_overlap=True,
            )

        # Transition: tom cascade into fracture
        if p.section.last_bar and p.section.next_section == "fracture":
    ...
    # ═══════════════════════════════════════════════════════════════
        ...
        p.cellular_1d(
            "kick_1", rule=110, velocity=44, no_overlap=True, probability=0.80
        )
        ...
        p.ghost_fill(
            "snare_1",
            density=0.28 * (0.5 + ghost_wander * 0.5),
            velocity=(26, 55),
            bias="offbeat",
            no_overlap=True,
        )
        ...
        if hat_feel > 0.4:
            oh_d = 0.10 * hat_feel
            p.bresenham_poly(
                parts={"hi_hat_open": oh_d},
                velocity={"hi_hat_open": 85},
            )
        ...
        if chaos_spark > 0.42:
            p.cellular_1d(
                "ride_1",
                rule=30,
                velocity=round(38 + 30 * chaos_spark),
                probability=0.58,
            )
    ...
    # ═══════════════════════════════════════════════════════════════
        ...
        if t >= 0.7:
            p.cellular_1d(
                "kick_1", rule=30, velocity=18, no_overlap=True, probability=0.10
            )
    ...
    if chaos_spark > 0.92 and section != "void":
        p.ghost_fill(
            "kick_1", density=0.32, velocity=(34, 55), bias="uniform", no_overlap=True
        )
        p.ghost_fill(
            "snare_1", density=0.22, velocity=(28, 48), bias="offbeat", no_overlap=True
        )
