...
def fetch_iss(p) -> None:
    ...
    try:
        ...
        if vis == "daylight":
            composition.harmony(
                style=CHORD_GRAPH_DAYLIGHT, cycle_beats=32, gravity=gravity
            )
        else:
            composition.harmony(
                style=CHORD_GRAPH_ECLIPSED, cycle_beats=32, gravity=gravity
            )
...
def drums(p):
    ...
    p.hit_steps(
        "kick_1", [0, 4, 8, 12], velocity=100, probability=1.0 - 0.4 * equator_prox
    )
...
def arp(p, chord):
    ...
    p.arpeggio(
        pitches, spacing=0.25, velocity=arp_velocity, duration=0.05, direction=direction
    )
