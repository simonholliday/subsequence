...
composition.conductor.lfo(
    "VELOCITY_SWELL", shape="sine", cycle_beats=16, min_val=55, max_val=105
)
...
def _compute_triggers(divisors, logic="OR"):
    ...
    if logic == "OR":
        ...
        merged = sorted(t for t in range(cycle) if sum(t in s for s in tick_sets) == 1)
...
seq1_divs = [
    d
    for d, on in [
        (RHYTHM1_DIV, RHYTHM1_SEQ1),
        (RHYTHM2_DIV, RHYTHM2_SEQ1),
        (RHYTHM3_DIV, RHYTHM3_SEQ1),
        (RHYTHM4_DIV, RHYTHM4_SEQ1),
    ]
    if on
]
...
seq2_divs = [
    d
    for d, on in [
        (RHYTHM1_DIV, RHYTHM1_SEQ2),
        (RHYTHM2_DIV, RHYTHM2_SEQ2),
        (RHYTHM3_DIV, RHYTHM3_SEQ2),
        (RHYTHM4_DIV, RHYTHM4_SEQ2),
    ]
    if on
]
...
if seq1_triggers:
    if SEQ1_ASSIGN_OSC1:
        ...
        @composition.pattern(
            channel=VCO1_CHANNEL, steps=seq1_cycle, step_duration=dur.QUARTER
        )
        def vco1(p):
    ...
    if SEQ1_ASSIGN_SUB1:
        ...
        @composition.pattern(
            channel=VCO1_SUB1_CHANNEL, steps=seq1_cycle, step_duration=dur.QUARTER
        )
        def vco1_sub1(p):
            vel = round(p.signal("VELOCITY_SWELL"))
            for trigger_idx, tick in enumerate(seq1_triggers):
    ...
    if SEQ1_ASSIGN_SUB2:
        ...
        @composition.pattern(
            channel=VCO1_SUB2_CHANNEL, steps=seq1_cycle, step_duration=dur.QUARTER
        )
        def vco1_sub2(p):
...
if seq2_triggers:
    if SEQ2_ASSIGN_OSC2:
        ...
        @composition.pattern(
            channel=VCO2_CHANNEL, steps=seq2_cycle, step_duration=dur.QUARTER
        )
        def vco2(p):
            vel = round(p.signal("VELOCITY_SWELL"))
    ...
    if SEQ2_ASSIGN_SUB1:
        ...
        @composition.pattern(
            channel=VCO2_SUB1_CHANNEL, steps=seq2_cycle, step_duration=dur.QUARTER
        )
        def vco2_sub1(p):
            vel = round(p.signal("VELOCITY_SWELL"))
            for trigger_idx, tick in enumerate(seq2_triggers):
    ...
    if SEQ2_ASSIGN_SUB2:
        ...
        @composition.pattern(
            channel=VCO2_SUB2_CHANNEL, steps=seq2_cycle, step_duration=dur.QUARTER
        )
        def vco2_sub2(p):
            vel = round(p.signal("VELOCITY_SWELL"))
            for trigger_idx, tick in enumerate(seq2_triggers):
