...
def _run() -> int:
    ...
    print(
        "Use it to discover note, CC, channel, and transport data from your controller."
    )
    print(
        "Note: MIDI channels are displayed 1-indexed (1-16) to match hardware panels."
    )
    ...
    def _on_message(message: typing.Any) -> None:
        ...
        if message.type == "clock":
            ...
            if elapsed >= clock_window_seconds:
                ...
                print(
                    f"clock: {ticks_per_second:.2f} ticks/s (≈{estimated_bpm:.1f} BPM) [updated every 5 seconds]"
                )
