"""Exploratory simulation tool — measures how key-gravity and NIR-strength
settings shape the root-pitch distribution and streaks produced by each
`HarmonicState` graph style.

Not a pytest file.  Run directly with:

        python scripts/simulation_gravity.py
"""
...
def analyze_style(
    ...
    top_2_pct = (
        (sorted_common[0][1] + sorted_common[1][1]) / total_steps
        if len(sorted_common) > 1
        else top_1_pct
    )
...
def run_simulation() -> None:
    """Print a table of results across styles, gravities, and NIR settings."""

    print(
        f"{'Style':<20} | {'Grav':<4} | {'NIR':<4} | {'Max Strk':<8} | {'Top 1%':<6} | {'Top 2%':<6} | {'Unique':<6}"
    )
    print("-" * 75)
        ...
        for gravity in [0.0, 0.5, 0.8, 1.0]:
            for nir in [0.0, 0.5, 1.0]:
                ...
                if res:
                    ...
                    flag = ""
                    if (
                        res["top_1_pct"] > MAX_TOP_1_PCT
                        or res["top_2_pct"] > MAX_TOP_2_PCT
                        or res["max_streak"] > MAX_STREAK
                    ):
                        flag = " FAIL"
                        failures += 1

                    print(
                        f"{style:<20} | {gravity:<4.1f} | {nir:<4.1f} | {res['max_streak']:<8} | {res['top_1_pct']:<6.2f} | {res['top_2_pct']:<6.2f} | {res['unique_roots']:<6}{flag}"
                    )
    ...
    if failures:
        print(
            f"RESULT: {failures} combination(s) exceeded thresholds (top_1>{MAX_TOP_1_PCT}, top_2>{MAX_TOP_2_PCT}, streak>{MAX_STREAK})"
        )
    else:
