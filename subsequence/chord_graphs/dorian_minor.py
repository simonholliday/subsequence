...
class DorianMinor(subsequence.chord_graphs.ChordGraph):
    ...
    def build(
        ...
        submediant_dim = subsequence.chords.Chord(
            root_pc=(key_pc + 9) % 12, quality="diminished"
        )
        ...
        graph.add_transition(
            subdominant, submediant_dim, WEIGHT_WEAK
        )  # IV → vi° (passing dim; vi° resolves to v / i)
    ...
    def gravity_sets(
        ...
        functional.add(
            subsequence.chords.Chord(root_pc=(key_pc + 7) % 12, quality="minor")
        )
        functional.add(
