...
class Chord:
    ...
    def tones(
        ...
        if count is not None:
            n = len(intervals)
            return [
                effective_root + intervals[i % n] + 12 * (i // n) for i in range(count)
            ]
...
def register_chord_quality(
    ...
    for old_suffix in [
        s
        for s, q in _SUFFIX_TO_QUALITY.items()
        if q == name and s not in _BUILTIN_SUFFIXES
    ]:
...
def parse_chord(name: str) -> Chord:
    ...
    if not stripped or stripped[0] not in "ABCDEFG":
        raise ValueError(
            f"Cannot parse chord name {name!r} — expected a root like 'C', 'F#', 'Bb' then a quality, e.g. 'Cm7'"
        )
    ...
    if root_name not in NOTE_NAME_TO_PC:
        raise ValueError(
            f"Cannot parse chord name {name!r} — unknown root {root_name!r}"
        )
    if suffix not in _SUFFIX_TO_QUALITY:
        known = ", ".join(repr(key) for key in sorted(_SUFFIX_TO_QUALITY) if key)
        raise ValueError(
            f"Cannot parse chord name {name!r} — unknown quality {suffix!r}. Known suffixes: {known}"
        )
