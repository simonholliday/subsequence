...
class FormState:
    ...
    def __init__(
        ...
        if loop:
            if at_end not in ("stop", "loop"):
                raise ValueError(
                    f"loop=True conflicts with at_end={at_end!r} — pass one or the other"
                )
            at_end = "loop"
        ...
        if isinstance(sections, dict):
            ...
            for name, (bars, transitions) in sections.items():
                if bars < 1:
                    raise ValueError(
                        f"Section '{name}' must last at least 1 bar, got {bars}"
                    )
            ...
            if start_name not in self._section_bars:
                raise ValueError(
                    f"Start section '{start_name}' not found in form definition"
                )

            self._current = subsequence.forms.Section(
                name=start_name, bars=self._section_bars[start_name]
            )
            self._pick_next()
            ...
            self._sequence = [
                subsequence.forms._coerce_section(element) for element in elements
            ]
    ...
    def _pick_next(self) -> None:
        ...
        if self._graph is not None:
            ...
            if current_name in self._terminal_sections:
                ...
                self._next_section_name = self._graph.choose_next(
                    current_name, self._rng
                )
            ...
            if self._queued_position is not None:
                ...
                self._next_section_name = (
                    self._sequence[upcoming].name
                    if upcoming is not None
                    else (
                        self._current.name
                        if self._at_end == "hold" and self._current is not None
                        else None
                    )
                )
    ...
    def advance(self) -> bool:
        ...
        assert self._current is not None, (
            "Form state invariant: current should not be None when not finished"
        )
        ...
        if self._bar_in_section >= self._current.bars:
            if self._graph is not None:
                ...
                self._current = subsequence.forms.Section(
                    name=next_name, bars=self._section_bars[next_name]
                )
                self._section_index += 1
                self._bar_in_section = 0
                self._pick_next()
    ...
    def jump_to(self, section_name: str) -> None:
        ...
        self._current = subsequence.forms.Section(
            name=section_name, bars=self._section_bars[section_name]
        )
        self._bar_in_section = 0
        self._section_index += 1
        self._finished = False
