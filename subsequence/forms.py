...
class Section:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.name, str) or not self.name:
            raise ValueError(
                f"a section needs a non-empty string name, got {self.name!r}"
            )
        ...
        if (
            ...
            raise ValueError(
                f"Section {self.name!r} must last at least 1 bar, got {self.bars!r}"
            )

        if not 0.0 <= float(self.energy) <= 1.0:
            raise ValueError(
                f"Section {self.name!r} energy must be 0.0–1.0, got {self.energy!r}"
            )
...
class Form:
    ...
    def replace(
        ...
        **changes: typing.Any,
        ...
        if section is None:
            if not changes:
                raise ValueError(
                    "replace() needs a Section or field changes (bars=, energy=, key=, name=)"
                )
            section = dataclasses.replace(self.sections[index], **changes)

        return Form(
            self.sections[:index]
            + (_coerce_section(section),)
    ...
    def insert(self, slot: int, section: typing.Any) -> "Form":
        ...
        if (
            ...
            or not 1 <= slot <= len(self.sections) + 1
        ):
            raise ValueError(
                f"slot {slot!r} is out of range (1–{len(self.sections) + 1})"
            )

        index = slot - 1

        return Form(
            self.sections[:index] + (_coerce_section(section),) + self.sections[index:],
            key=self.key,
            scale=self.scale,
        )
    ...
    def with_energy(self, energies: typing.Dict[str, float]) -> "Form":
        ...
        for name in energies:
            if name not in names:
                ...
                raise ValueError(
                    f"with_energy: no section named {name!r} in this form (sections: {known})"
                )

        return Form(
            tuple(
                dataclasses.replace(section, energy=energies[section.name])
                if section.name in energies
                else section
    ...
    def describe(self) -> str:
        ...
        for slot, section in enumerate(self.sections, start=1):
            ...
            lines.append(
                f"  {slot}. bars {bar}–{bar + section.bars - 1}  {section.name:<10} ({section.bars} bars){extras}"
            )
            bar += section.bars
