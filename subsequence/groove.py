...
class Groove:
    ...
    def __post_init__(self) -> None:
        ...
        if self.velocities is not None and not self.velocities:
            raise ValueError(
                "velocities must not be empty (use None for no velocity adjustment)"
            )
    ...
    def from_agr(path: str, grid: typing.Optional[float] = None) -> "Groove":
        ...
        **What is NOT imported:**
        ...
        if has_velocity_variation and max_vel > 0:
            raw_scales = [
                (v / max_vel) if v is not None else 1.0 for v in slot_velocities
            ]
...
def _scale_step_velocity(
    step: "subsequence.pattern.Step", scale: float
) -> "subsequence.pattern.Step":
    """
