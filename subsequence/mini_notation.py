...
def _parse_recursive(
    tokens: list, start_time: float, duration: float
) -> typing.List[ParsedEvent]:
    """
            ...
            if "?" in token and token[0] != "?":
                ...
                try:
                    ...
                    raise MiniNotationError(
                        f"Invalid probability suffix in {token!r} - expected a number after '?'"
                    )

                if not 0.0 <= probability <= 1.0:
                    raise MiniNotationError(
                        f"Probability in {token!r} must be between 0.0 and 1.0"
                    )
            else:
                symbol = token
                probability = 1.0
...
def _post_process_sustains(
    ...
    for event in events:
        if event.symbol == "_":
            ...
            if (
                last_event
                and abs((last_event.time + last_event.duration) - event.time) < 1e-9
            ):
