...
class PatternMidiMixin:
    ...
    def _ramp_pulses(
        ...
        self._ramp_pulse_span(
            pulse_start, pulse_end, start, end, shape, resolution, event_fn
        )

    # ── CC messages ─────────────────────────────────────────────────────────

    def cc(
        self, control: typing.Union[int, str], value: int, beat: float = 0.0
    ...
    def cc_ramp(
        ...
        self._ramp_pulses(
            beat_start, beat_end, float(start), float(end), shape, resolution, _event
        )
        return typing.cast("subsequence.pattern_builder.PatternBuilder", self)
    ...
    def pitch_bend(
        self, value: float, beat: float = 0.0
    ) -> "subsequence.pattern_builder.PatternBuilder":
        """
        ...
        self._pattern.cc_events.append(
            subsequence.pattern.CcEvent(
                pulse=pulse, message_type="pitchwheel", value=midi_value
            )
        )
        return typing.cast("subsequence.pattern_builder.PatternBuilder", self)
    ...
    def _append_param_select(
        self, pulse: int, parameter: int, msb_cc: int, lsb_cc: int
    ) -> None:
    ...
    def _append_data_entry(self, pulse: int, value: int, fine: bool) -> None:
        ...
        if fine:
            ...
            if not 0 <= value <= 127:
                raise ValueError(
                    f"NRPN/RPN value must be 0–127 when fine=False, got {value}"
                )
            value_msb = value
            value_lsb = None
    ...
    def _validate_ramp_endpoints(self, start: int, end: int, fine: bool) -> None:
        ...
        for label, value in (("start", start), ("end", end)):
            if not 0 <= value <= limit:
                raise ValueError(
                    f"NRPN/RPN ramp {label} must be 0–{limit} (fine={fine}), got {value}"
                )
    ...
    def nrpn(
        ...
        self._append_param_select(
            pulse, param, pymididefs.cc.NRPN_MSB, pymididefs.cc.NRPN_LSB
        )
    ...
    def rpn(
        ...
        self._append_param_select(
            pulse, param, pymididefs.cc.RPN_MSB, pymididefs.cc.RPN_LSB
        )
        self._append_data_entry(pulse, value, fine)
    ...
    def nrpn_ramp(
        ...
        **Mid-ramp parameter persistence:** between ``beat_start`` and
        ...
        self._append_param_select(
            int(beat_start * subsequence.constants.MIDI_QUARTER_NOTE),
            param,
            pymididefs.cc.NRPN_MSB,
            pymididefs.cc.NRPN_LSB,
        )
    ...
    def rpn_ramp(
        ...
        self._append_param_select(
            int(beat_start * subsequence.constants.MIDI_QUARTER_NOTE),
            param,
            pymididefs.cc.RPN_MSB,
            pymididefs.cc.RPN_LSB,
        )
    ...
    def sysex(
        self, data: typing.Union[bytes, typing.List[int]], beat: float = 0.0
    ) -> "subsequence.pattern_builder.PatternBuilder":
        """
        ...
        self._pattern.cc_events.append(
            subsequence.pattern.CcEvent(
                pulse=pulse, message_type="sysex", data=bytes(data)
            )
        )
        return typing.cast("subsequence.pattern_builder.PatternBuilder", self)
    ...
    def osc(
        self, address: str, *args: typing.Any, beat: float = 0.0
    ) -> "subsequence.pattern_builder.PatternBuilder":
        """
        ...
        self._pattern.osc_events.append(
            subsequence.pattern.OscEvent(pulse=pulse, address=address, args=args)
        )
        return typing.cast("subsequence.pattern_builder.PatternBuilder", self)
    ...
    def osc_ramp(
        ...
        def _event(pulse: int, val: float) -> None:
            self._pattern.osc_events.append(
                subsequence.pattern.OscEvent(pulse=pulse, address=address, args=(val,))
            )
    ...
    def _generate_bend_events(
        ...
        self._ramp_pulse_span(
            pulse_start, pulse_end, start_value, end_value, shape, resolution, _event
        )
    ...
    def bend(
        ...
        *after* ``legato()`` / ``detached()`` / ``duration()`` so that note durations are final.
        ...
        self._generate_bend_events(
            0.0, amount, bend_start_pulse, bend_end_pulse, resolution, shape
        )
    ...
    def portamento(
        ...
        *after* ``legato()`` / ``detached()`` / ``duration()`` so that note durations are final.
        ...
        for i in range(n):
            ...
            self._generate_bend_events(
                0.0, amount, glide_start_pulse, glide_end_pulse, resolution, shape
            )
    ...
    def slide(
        ...
        if notes is not None and steps is not None:
            raise ValueError(
                "slide() takes notes= or steps=, not both — they name the same slide targets two different ways"
            )
        ...
        if notes is not None:
            ...
            for idx in notes:
                if not -n <= idx < n:
                    raise ValueError(
                        f"slide() note index {idx} is outside this pattern's {n} notes"
                    )
            ...
            for s in steps or []:
                flagged.add(
                    int(s * step_beats * subsequence.constants.MIDI_QUARTER_NOTE)
                )
