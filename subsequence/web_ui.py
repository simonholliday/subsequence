...
class WebUI:
        ...
        class Handler(http.server.SimpleHTTPRequestHandler):
            ...
            def do_GET(self) -> None:
                ...
                if self.path in ("/", "/index.html"):
                    try:
                        with open(
                            os.path.join(web_dir, "index.html"), "r", encoding="utf-8"
                        ) as fh:
    ...
    async def _broadcast_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.error(
                    f"Error broadcasting UI state: {e}\n{traceback.format_exc()}"
                )
    ...
    def _get_state(self, comp: typing.Any) -> typing.Dict[str, typing.Any]:
        ...
        def _extract_val(val: typing.Any) -> typing.Optional[float]:
            if hasattr(val, "current"):  # Matches EasedValue
                try:
                    ...
                    logger.debug(
                        f"WebUI failed to extract float from .current on {val}: {e}"
                    )
            if callable(getattr(val, "value", None)):
                try:
                    ...
                    logger.debug(
                        f"WebUI failed to extract float from .value() on {val}: {e}"
                    )
        ...
        if comp.conductor:
            beat_time = (
                comp.sequencer.pulse_count / comp.sequencer.pulses_per_beat
                if comp.sequencer
                else 0.0
            )
