...
def test_progression_cadence_strong_in_minor_is_the_major_dominant () -> None:
	...
	value = subsequence.progression([1, 6, 4, 5]).cadence("strong").resolve("A", "aeolian")
...
def test_generate_cadence_pins_the_tail () -> None:
	...
	value = subsequence.Progression.generate(
		style="functional_major", bars=4, key="C", seed=3, cadence="strong",
	)
...
def test_generate_cadence_soft_is_walkable () -> None:
	...
	value = subsequence.Progression.generate(
		style="functional_major", bars=4, key="C", seed=3, cadence="soft",
	)
...
def test_generate_cadence_fakeout_swerves () -> None:
	...
	value = subsequence.Progression.generate(
		style="functional_major", bars=4, key="C", seed=3, cadence="fakeout",
	)
...
def test_generate_cadence_key_relative_value () -> None:
	...
	value = subsequence.Progression.generate(
		style="aeolian_minor", bars=4, seed=3, cadence="strong",
	)
...
def test_generate_cadence_conflicts_raise () -> None:
	...
	with pytest.raises(ValueError, match="conflicts with end"):
		subsequence.Progression.generate(
			style="functional_major", bars=4, key="C", seed=1, cadence="strong", end="V",
		)

	with pytest.raises(ValueError, match=r"pins\[3\]"):
		subsequence.Progression.generate(
			style="functional_major", bars=4, key="C", seed=1, cadence="strong", pins={3: "F"},
		)

	with pytest.raises(ValueError, match="needs 2 bars"):
		subsequence.Progression.generate(
			style="functional_major", bars=1, key="C", seed=1, cadence="strong",
		)
...
def test_progression_factory_passes_cadence_through () -> None:
	...
	value = subsequence.progression(style="functional_major", key="C", bars=4, seed=3, cadence="strong")
...
def test_freeze_cadence_pins_the_tail (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C", seed=5)
...
def test_constraint_scale_inferred_from_style (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="A", seed=5)
...
async def _capture_clock (**kwargs: typing.Any) -> typing.Tuple[
...
async def test_request_cadence_unwalkable_falls_back_to_fiat (
	...
	assert hs.current_chord.name() == "F#"		# fiat still commits — the engine stays coherent
...
def test_aeolian_minor_cadential_edges_are_labelled () -> None:
	...
	hs = subsequence.harmonic_state.HarmonicState(key_name="A", graph_style="aeolian_minor")
...
def test_sentence_shape_and_close () -> None:
	...
	assert [e.beat for e in value.segments[2].events] == [e.beat for e in value.segments[0].events]
