...
async def test_fires_at_lookahead_before_boundary(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        clock, start_pulse=8 * ppb, reschedule_lookahead=1
    )
...
async def test_receives_boundary_pulse_not_fire_pulse(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        clock, start_pulse=4 * ppb, reschedule_lookahead=2
    )
...
async def test_variable_intervals_walk_irregular_spans(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        clock, start_pulse=0, reschedule_lookahead=1
    )
...
async def test_failing_callback_is_dropped_and_isolated(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        bad, start_pulse=0, reschedule_lookahead=1
    )
    await sequencer.schedule_callback_sequence(
        good, start_pulse=0, reschedule_lookahead=1
    )
...
async def test_tiny_interval_clamps_and_stays_monotonic(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        clock, start_pulse=0, reschedule_lookahead=0
    )
...
@pytest.mark.asyncio
async def test_fixed_callbacks_fire_before_sequences_at_same_pulse(
    patch_midi: None,
) -> None:
    ...
    await sequencer.schedule_callback_sequence(
        harmony_clock, start_pulse=4 * ppb, reschedule_lookahead=1
    )
    await sequencer.schedule_callback_repeating(
        form_clock, interval_beats=4, start_pulse=4 * ppb, reschedule_lookahead=1
    )
...
@pytest.mark.asyncio
async def test_sequences_fire_before_pattern_rebuilds_at_same_pulse(
    patch_midi: None,
) -> None:
...
async def test_negative_lookahead_raises(patch_midi: None) -> None:
    ...
    with pytest.raises(ValueError):
        await sequencer.schedule_callback_sequence(
            lambda boundary: None, reschedule_lookahead=-1
        )
