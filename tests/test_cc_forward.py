...
def test_callable_transform(patch_midi: None) -> None:
    ...
    comp.cc_forward(
        1,
        lambda v, ch: mido.Message(
            "control_change", channel=ch, control=74, value=v // 2
        ),
    )
    transform = comp._cc_forwards[0]["transform"]
    msg = transform(100, 0)
    ...
    seq.cc_forwards = [
        {
            ...
            "channel": None,
            "mode": "instant",
            "transform": lambda v, ch: mido.Message(
                "control_change", channel=ch, control=74, value=v
            ),
        }
    ]
    ...
    seq.cc_forwards = [
        {
            ...
            "mode": "instant",
            "transform": lambda v, ch: mido.Message(
                "control_change", channel=ch, control=1, value=v
            ),
        }
    ]
...
async def test_queued_forward_routes_to_output_device(patch_midi: None) -> None:
    ...
    seq.cc_forwards = [
        {
            ...
            "channel": None,
            "mode": "queued",
            "transform": lambda value, channel: mido.Message(
                "control_change", channel=0, control=74, value=value
            ),
            "input_device": None,
            "output_device": 1,
        }
    ...
    seq._on_midi_input(
        mido.Message("control_change", channel=0, control=74, value=99), device_idx=0
    )
    ...
    assert not any(
        m.type == "control_change" and m.value == 99 for m in spy_primary.sent
    )
...
def test_callable_with_output_channel_rechannels(patch_midi: None) -> None:
    ...
    comp.cc_forward(
        1,
        lambda value, channel: mido.Message(
            "control_change", channel=channel, control=10, value=value
        ),
        output_channel=5,
    )
