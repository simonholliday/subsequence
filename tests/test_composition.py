...
def test_chord_injection(patch_midi: None) -> None:
    ...
    composition.harmony(style="turnaround_global", cycle_beats=4, dominant_7th=True)
...
def test_seed_produces_deterministic_patterns(patch_midi: None) -> None:
    ...
    def build_steps(seed: int) -> set:
        composition = subsequence.Composition(
            output_device="Dummy MIDI", bpm=120, seed=seed
        )
...
def test_pattern_lookahead_is_never_clamped(patch_midi: None) -> None:
    ...
    composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C")
    composition.harmony(
        style="diatonic_major", cycle_beats=4, reschedule_lookahead=0.25
    )
...
def test_freeze_chord_count(patch_midi: None) -> None:
    ...
    for bars in (1, 4, 8):
        ...
        assert len(prog.chords) == bars, (
            f"Expected {bars} chords, got {len(prog.chords)}"
        )
...
async def test_section_change_restarts_the_progression(patch_midi: None) -> None:
    ...
    cb, horizon, _ = await _capture_clock(
        get_harmonic_state=lambda: hs,
        cycle_beats=4,
        get_section_progression=lambda: (
            current_section[0],
            current_section[1],
            4,
            prog,
    ...
    prog = subsequence.progressions.Progression(
        spans=(
            subsequence.progressions.ChordSpan(
                chord=subsequence.chords.parse_chord("Am"), beats=4.0
            ),
        ),
...
@pytest.mark.asyncio
async def test_exhausted_section_falls_through_to_live_with_style(
    patch_midi: None,
) -> None:
...
async def test_bound_progression_loops_without_a_live_engine(patch_midi: None) -> None:
    ...
    assert (
        horizon.chord_at(101.0).name() == "F"
    )  # beat 101 → offset 5 in the 8-beat loop
...
@pytest.mark.asyncio
async def test_live_window_clamps_beyond_planned_with_one_warning(
    patch_midi: None,
) -> None:
...
@pytest.mark.asyncio
async def test_decorated_spans_reach_the_window_engine_keeps_bare_triads(
    patch_midi: None,
) -> None:
    ...
    assert hs.current_chord == subsequence.chords.parse_chord(
        "Am"
    )  # bare in the engine
...
def test_one_indexed_channels_subtracts_one(patch_midi: None) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=120, zero_indexed_channels=False
    )
...
def test_zero_indexed_channels_rejects_16(patch_midi: None) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=120, zero_indexed_channels=True
    )
...
def test_resolve_length_bars_one(patch_midi: None) -> None:
    ...
    beat_length, default_grid = subsequence.Composition._resolve_length(
        None, 1, None, None
    )
...
def test_section_chords_replay_aligns_with_section_boundaries(
    tmp_path, patch_midi: None
) -> None:
    ...
    assert all(
        chorus_chords[i] == frozen_names[i % 2] for i in range(len(chorus_chords))
    )
    assert len(verse_chords) >= 2


@pytest.mark.asyncio
async def test_unregistered_pattern_is_not_resurrected_by_later_graduation(
    patch_midi: None,
) -> None:
...
def test_freeze_unsatisfiable_raises_and_engine_state_survives(
    patch_midi: None,
) -> None:
