...
def _make(seed: int = 42, key: str = "A") -> "subsequence.Composition":
    ...
    return subsequence.Composition(
        output_device="Dummy MIDI", bpm=120, key=key, seed=seed
    )
...
def test_duplicate_pattern_name_warns(
    ...
    assert any(
        "Duplicate pattern name 'clash'" in record.message for record in caplog.records
    )
...
def test_reroll_bumps_nonce_and_prints_effective_seed(
    patch_midi: None, capsys: pytest.CaptureFixture
) -> None:
...
def test_reroll_unseeded_explains(
    patch_midi: None, capsys: pytest.CaptureFixture
) -> None:
...
def test_lock_refuses_reroll_until_unlock(
    patch_midi: None, capsys: pytest.CaptureFixture
) -> None:
