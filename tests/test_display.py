...
def test_start_installs_handler(patch_midi: None) -> None:
    ...
    try:
        ...
        assert isinstance(
            root_logger.handlers[0], subsequence.display.DisplayLogHandler
        )
    finally:
        display.stop()
...
def test_section_display_syncs_with_bar_counter(patch_midi: None) -> None:
    ...
    assert "[intro 1/4 \u2192 verse]" in status_2, (
        "Section should NOT update before bar counter changes"
    )
    ...
    assert "[intro 2/4 \u2192 verse]" in status_3, (
        "Section should update when bar counter changes"
    )
...
def test_grid_build_drum_pattern(
...
def test_grid_build_pitched_pattern(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_sustain_single_note(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_sustain_short_note(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_sustain_does_not_overwrite_attack(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_pitched_summary_sustain(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_scale_doubles_columns(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_scale_on_grid_dot_between_grid_space(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    pat = subsequence.pattern.Pattern(channel=5, length=4)
    pat.add_note(
        position=0, pitch=60, velocity=100, duration=3
    )  # short note, no sustain
    pat._drum_note_map = None  # type: ignore[attr-defined]
    pat._default_grid = 16  # type: ignore[attr-defined]
    pat._muted = False  # type: ignore[attr-defined]
...
def test_grid_scale_sustain_fills_between_grid(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    for i in range(1, 8):
        assert individual_cells[i] == ">", (
            f"col {i} should be sustain '>', got '{individual_cells[i]}'"
        )
...
def test_grid_scale_float_snaps(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_grid_scale_uniform_spacing(
    patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    for scale, expected_step in [(1.0, 1), (2.0, 2), (3.0, 3), (1.5, 2), (2.6, 3)]:
        ...
        expected_markers = [
            i for i in range(len(individual_cells)) if i % expected_step == 0
        ]
        ...
        assert individual_cells[8 * expected_step] == "▓", (
            f"scale={scale}"
        )  # velocity 80
        assert individual_cells[12 * expected_step] == "▒", (
            f"scale={scale}"
        )  # velocity 60
