...
def test_form_coerces_tuples_and_sections() -> None:
    ...
    form = subsequence.Form(
        [("verse", 8), subsequence.Section("chorus", 8, energy=0.9)]
    )
...
def test_form_describe_prints_slots_and_payload() -> None:
    ...
    assert (
        "verse" in text
        and "bars 1–8" in text
        and "energy=0.6" in text
        and "key=D" in text
    )
...
def test_at_end_hold_repeats_the_final_section() -> None:
    ...
    state = subsequence.form_state.FormState(
        [("verse", 1), ("outro", 1)], at_end="hold"
    )
    ...
    info = state.get_section_info()
    assert (
        info is not None and info.name == "outro" and info.index == 2 and info.bar == 0
    )
...
def test_sequence_jump_to_lands_on_next_occurrence() -> None:
    ...
    state = subsequence.form_state.FormState(
        [("verse", 4), ("chorus", 4), ("verse", 4)]
    )

    state.jump_to("chorus")
    info = state.get_section_info()
    assert (
        info is not None and info.name == "chorus" and info.bar == 0 and info.index == 1
    )
...
def test_form_freeze_walks_to_the_terminal(patch_midi: None) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=120, key="C", seed=1337
    )
    composition.form(
        {
            "intro": (8, [("verse", 1)]),
...
def test_form_freeze_reproduces_the_live_walk(patch_midi: None) -> None:
    ...
    def build() -> subsequence.composition.Composition:
        composition = subsequence.Composition(
            output_device="Dummy MIDI", bpm=120, key="C", seed=7
        )
        composition.form(
            {
                "a": (1, [("b", 1), ("c", 1)]),
...
def test_form_freeze_bounds_and_validation(patch_midi: None) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=120, key="C", seed=3
    )
    composition.form(
        {
            "a": (4, [("b", 1)]),
...
def test_energy_priority_dict_over_payload_over_default(patch_midi: None) -> None:
    ...
    info = (
        composition._form_state.get_section_info() if composition._form_state else None
    )
...
def test_energy_ramp_interpolates_across_the_section(patch_midi: None) -> None:
    ...
    assert composition._current_energy(state.get_section_info()) == pytest.approx(
        0.2
    )  # bar 0 = start
...
def test_min_energy_gates_the_pattern(patch_midi: None) -> None:
    ...
    @composition.pattern(
        channel=10, beats=4, drum_note_map={"conga": 63}, min_energy=0.8
    )
...
async def test_section_event_fires_on_changes() -> None:
    ...
    mock_seq.events = subsequence.event_emitter.EventEmitter()
    mock_seq.events.on(
        "section", lambda info: heard.append(info.name if info else None)
    )
...
async def test_on_bar_hook_receives_boundary_pulses() -> None:
    ...
    cb, _events = await _capture_form_clock(
        state, on_bar=lambda pulse, changed: calls.append((pulse, changed))
    )
...
def test_transition_validation(patch_midi: None) -> None:
    ...
    with pytest.raises(ValueError, match="channel="):
        composition.transition(
            before="*", fill=subsequence.Motif.hits("snare", beats=[0], length=1)
        )
...
def test_transition_device_name_defers_resolution(patch_midi: None) -> None:
    ...
    composition.transition(
        before="*", fill=subsequence.motif([1]), channel=10, device="Aux Synth"
    )
...
def test_transition_fill_fires_in_the_final_bar(patch_midi: None) -> None:
    ...
    composition._schedule_one_shot = lambda pattern, start_pulse: fired.append(
        (pattern, start_pulse)
...
def test_transition_star_skips_repeats(patch_midi: None) -> None:
    ...
    composition.transition(
        before="*",
        fill=subsequence.Motif.hits("snare", beats=[0], length=1),
        channel=10,
...
def test_transition_mute_closes_and_reopens(patch_midi: None) -> None:
    ...
    assert (
        performer_muted._muted is True
    )  # the performer's mute was never ours to touch
...
def test_sketch_e_full_song_runs_as_written(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=480, key="C", scale="major", seed=42
    )

    S = subsequence.Section
    form = subsequence.Form(
        [
            S("verse", bars=8, energy=0.55),
            S("verse", bars=8, energy=0.6),
            S("chorus", bars=8, energy=0.9),
            S("verse", bars=8, energy=0.65),
    ...
    chorus_prog = subsequence.progression(
        style="functional_major", bars=8, end=1, seed=9
    )

    theme = subsequence.motif([1, 3, 5, 6, 5, 3, 2], durations=[1, 1, 1, 1, 1, 1, 2])
    verse_line = subsequence.Phrase.develop(
        theme, bars=8, plan=["a", "a", "a", "b"], seed=4
    )

    print(verse_prog)
    print(verse_line)
    verse_prog = verse_prog.replace(6, "Dm9")
    verse_line = verse_line.replace(
        4, subsequence.motif([3, 2, 1], durations=[1, 1, 2])
    )
...
def test_sketch_f_fully_generated_from_one_seed(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
    ...
    composition = subsequence.Composition(
        output_device="Dummy MIDI", bpm=480, key="F#", scale="minor", seed=1337
    )
    ...
    composition.energy(
        {"intro": 0.2, "verse": 0.55, "drop": 0.95, "break": 0.35, "outro": 0.2}
    )
    ...
    hook = subsequence.Motif.generate(
        rhythm=[0, 1, 1.5, 1.75, 2.5],
        scale="minor_pentatonic",
        contour="arch",
        end_on=1,
        seed=composition.seed_for("hook"),
    )
    verse_line = subsequence.Phrase.develop(
        hook, bars=8, plan="call_response", seed=composition.seed_for("verse_line")
    )
    drop_line = subsequence.Phrase.develop(
        hook.transpose(steps=2),
        bars=8,
        plan=["a", "a", "a", "b"],
        seed=composition.seed_for("drop_line"),
    )
    ...
    for track in mid.tracks:
        ...
        for msg in track:
            now += msg.time
            if (
                isinstance(msg, mido.MetaMessage)
                or msg.type != "note_on"
                or msg.velocity == 0
            ):
                continue
    ...
    assert drum_notes > 0
    assert (
        lead_beats and min(lead_beats) >= 32.0
    )  # silent intro; the lead enters with the verse
