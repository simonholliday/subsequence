...
def _make_steps(*pulses: int, velocity: int = 100) -> dict:
    ...
    steps = {}
    for p in pulses:
        steps[p] = subsequence.pattern.Step(
            notes=[
                subsequence.pattern.Note(
                    pitch=60, velocity=velocity, duration=6, channel=0
                )
            ]
        )
    return steps
...
def test_apply_groove_velocity_scaling() -> None:
    ...
    g = subsequence.groove.Groove(offsets=[0.0, 0.0], grid=0.25, velocities=[1.0, 0.5])
...
def test_from_agr_parses_swing_16ths_57() -> None:
    ...
    for i in range(1, 16, 2):
        assert abs(g.offsets[i] - 0.035) < 0.001, (
            f"Odd slot {i} offset should be ~0.035"
        )
...
def _write_agr(
    path: str,
    timing_amount: float = 100.0,
    velocity_amount: float = 100.0,
    note_times_and_velocities=None,
) -> None:
...
def test_apply_groove_strength_zero_no_change() -> None:
    ...
    result = subsequence.groove.apply_groove(
        steps, g, pulses_per_quarter=24, strength=0.0
    )
...
def test_apply_groove_strength_half_timing() -> None:
    ...
    result_full = subsequence.groove.apply_groove(
        steps, g, pulses_per_quarter=24, strength=1.0
    )
    result_half = subsequence.groove.apply_groove(
        steps, g, pulses_per_quarter=24, strength=0.5
    )
...
def test_apply_groove_strength_velocity_blend() -> None:
    ...
    result_zero = subsequence.groove.apply_groove(
        steps, g, pulses_per_quarter=24, strength=0.0
    )
