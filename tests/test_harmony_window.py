...
def test_render_bound_progression_drives_the_chord_pattern (tmp_path: pathlib.Path, patch_midi: None) -> None:
...
def test_render_exposes_p_harmony_and_anticipation (tmp_path: pathlib.Path, patch_midi: None) -> None:
	...
	seen: typing.List[typing.Tuple[str, typing.Optional[str], typing.Optional[float], str]] = []

	@composition.pattern(channel=1, beats=4)
	def watcher (p, chord) -> None:
		next_name = chord.next.name() if chord.next is not None else None
		seen.append((p.harmony.chord.name(), next_name, chord.beats_remaining, p.harmony.chord_at(2.0).name()))
		p.note(60, beat=0)
...
def test_render_variable_harmonic_rhythm (tmp_path: pathlib.Path, patch_midi: None) -> None:
...
def test_render_chord_tones_follow_the_changes (tmp_path: pathlib.Path, patch_midi: None) -> None:
	...
	riff = subsequence.Motif.from_events([
		subsequence.MotifEvent(beat=0.0, pitch=subsequence.ChordTone("root"), duration=1.0),
		subsequence.MotifEvent(beat=2.0, pitch=subsequence.ChordTone("root"), duration=1.0),
	], length=4)
...
def test_render_section_progressions_with_form (tmp_path: pathlib.Path, patch_midi: None) -> None:
...
def test_min_span_below_lookahead_raises_at_play (tmp_path: pathlib.Path, patch_midi: None) -> None:
