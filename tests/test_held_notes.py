...
def _make_seq (release_ms: float = 0.0, latch: bool = False, channel: typing.Optional[int] = None, device: typing.Optional[int] = None) -> subsequence.sequencer.Sequencer:
	...
	seq._held_notes = subsequence.held_notes.HeldNotes(release_ms=release_ms, latch=latch)
...
def test_on_midi_input_velocity_zero_is_note_off () -> None:
	...
	seq = _make_seq()
	seq._on_midi_input(mido.Message('note_on', channel=0, note=60, velocity=0), device_idx=0)
...
def _builder (held: typing.Optional[subsequence.held_notes.HeldNotes] = None) -> subsequence.pattern_builder.PatternBuilder:
	pattern = subsequence.pattern.Pattern(channel=0, length=4)
	return subsequence.pattern_builder.PatternBuilder(pattern=pattern, cycle=0, default_grid=16, held_notes=held)
