...
def _placed_pitches (composition: "subsequence.composition.Composition", index: int = 0) -> typing.List[int]:

	"""Build the pending pattern at *index* and return the MIDI note numbers it placed."""

	pattern = composition._build_pattern_from_pending(composition._pending_patterns[index])
...
def test_chord_name_progression_ignores_section_key (patch_midi: None) -> None:
	...
	info = subsequence.form_state.SectionInfo(name="verse", bar=0, bars=4, index=0, key="A")
	resolved = composition._resolve_section_progression(info)
	assert resolved is not None
	assert [c.name() for c in resolved.chords] == ["Am", "F"]		# still A minor / F, not transposed
...
def test_effective_key_precedence () -> None:
	...
	comp = subsequence.composition.Composition.__new__(subsequence.composition.Composition)
	comp.key = "C"
	comp.scale = "major"
	comp._form_key = None
	...
	assert comp._effective_key_scale(S("v", 0, 4, 0)) == ("G", "major")		# scale falls to composition

	# Section overrides form; key and scale independent.
	assert comp._effective_key_scale(S("v", 0, 4, 0, key="A", scale="minor")) == ("A", "minor")
	assert comp._effective_key_scale(S("v", 0, 4, 0, key="A")) == ("A", "major")		# tonic moves, mode stays
...
def test_section_mode_travels (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C", scale="major", seed=1)
	composition.form(subsequence.Form([subsequence.Section("verse", 4, key="A", scale="minor")]))
...
def test_form_value_carries_its_own_key (patch_midi: None) -> None:
	...
	composition.form(subsequence.Form([subsequence.Section("verse", 4)], key="E"), key="G")
...
def test_relative_section_progression_re_keys (tmp_path: pathlib.Path, patch_midi: None) -> None:

	"""The truck-driver's modulation: same numbered progression, two keys, two sections."""

	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=480, key="C", scale="major", seed=1)
...
def test_section_key_moves_melody_and_chords_together (tmp_path: pathlib.Path, patch_midi: None) -> None:
	...
	def lead (p, chord) -> None:
		...
		p.motif(subsequence.motif([1]), root=60)
		roots.extend(note.pitch for step in p._pattern.steps.values() for note in step.notes)
...
def test_section_fall_through_uses_composition_key (patch_midi: None) -> None:
	...
	composition.section_chords("verse", subsequence.progression([1, 4]))		# only 2 bars of an 8-bar section

	info = subsequence.form_state.SectionInfo(name="verse", bar=0, bars=8, index=0, key="D")
	resolved = composition._resolve_section_progression(info)

	assert resolved is not None
	assert [c.name() for c in resolved.chords] == ["D", "G"]		# the written bars are in D
	# Bars 3-8 fall through to the live engine, which is fixed in C (documented).
	assert composition._harmonic_state is not None
	assert composition._harmonic_state.key_root_pc == subsequence.chords.key_name_to_pc("C")
...
def test_relative_section_no_key_raises_at_run (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, seed=1)	# no key
...
def test_re_keying_is_deterministic (tmp_path: pathlib.Path, patch_midi: None) -> None:
	...
	def render (path: str) -> bytes:
		composition = subsequence.Composition(output_device="Dummy MIDI", bpm=480, key="C", scale="major", seed=7)
		composition.form(subsequence.Form([subsequence.Section("v", 4, key="A", scale="minor")]))
...
def test_resolution_cache_reuses_one_realisation (patch_midi: None) -> None:
	...
	info = subsequence.form_state.SectionInfo(name="verse", bar=0, bars=4, index=0, key="A", scale="minor")
...
def test_trigger_honours_the_key (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="D", seed=1)
...
def test_transition_fill_uses_section_key (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C", seed=1)
	composition.form(subsequence.Form([subsequence.Section("verse", 2, key="A"), subsequence.Section("chorus", 2)]))
...
def test_pin_re_keys_to_the_pinned_bars_section_not_the_playhead (patch_midi: None) -> None:
	...
	assert composition._resolve_pin(9).root_pc == subsequence.chords.key_name_to_pc("Bb")
...
def test_resolve_error_skips_section_not_crashes (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C", scale="major", seed=1)
	composition.form(subsequence.Form([subsequence.Section("verse", 4, scale="minor_pentatonic")]))
	composition.section_chords("verse", subsequence.progression([1, 7]))		# 7 is out of a 5-note scale
...
def test_resolve_error_safety_net_returns_none (patch_midi: None) -> None:
	...
	info = subsequence.form_state.SectionInfo(name="verse", bar=0, bars=4, index=0, scale="minor_pentatonic")
...
def test_form_freeze_carries_form_key (patch_midi: None) -> None:
	...
	composition = subsequence.Composition(output_device="Dummy MIDI", bpm=120, key="C", seed=1)
	composition.form({"intro": (4, [("verse", 1)]), "verse": (4, None)}, start="intro", key="A", scale="minor")
