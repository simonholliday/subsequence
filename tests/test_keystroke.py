...
@pytest.fixture
def fake_terminal (monkeypatch: pytest.MonkeyPatch) -> typing.List[typing.Tuple[int, int, typing.Any]]:
	...
	monkeypatch.setattr(termios, "tcsetattr", lambda fd, when, settings: restored.append((fd, when, settings)))
	monkeypatch.setattr(tty, "setcbreak", lambda fd: None)
	monkeypatch.setattr(select, "select", lambda rlist, wlist, xlist, timeout=None: ([], [], []))
	monkeypatch.setattr(sys, "stdin", _FakeStdin())

	return restored


def test_start_stop_joins_thread_and_restores_terminal (fake_terminal: typing.List[typing.Tuple[int, int, typing.Any]]) -> None:
...
def test_stop_without_start_is_noop (fake_terminal: typing.List[typing.Tuple[int, int, typing.Any]]) -> None:
