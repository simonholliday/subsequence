...
def _push_note(
    seq: subsequence.sequencer.Sequencer, device: int, note: int = 60, pulse: int = 0
) -> None:
    seq._push_event(
...
async def test_slow_device_sends_immediately(patch_midi: None) -> None:
    ...
    seq.midi_out = spy0  # fast
    spy1 = conftest.SpyMidiOut()
    seq.add_output_device("slow", spy1, latency_ms=20)  # slowest
...
async def test_fast_device_deferred_then_fires(patch_midi: None) -> None:
    ...
    spy0 = conftest.SpyMidiOut()
    seq.midi_out = spy0
    seq.set_device_latency(0, 0)  # fast
    spy1 = conftest.SpyMidiOut()
    seq.add_output_device("slow", spy1, latency_ms=10)  # slowest
...
async def test_two_devices_same_pulse_split(patch_midi: None) -> None:
    ...
    spy0 = conftest.SpyMidiOut()
    seq.midi_out = spy0
    seq.set_device_latency(0, 0)
    spy1 = conftest.SpyMidiOut()
    seq.add_output_device("slow", spy1, latency_ms=10)
...
async def test_intra_device_order_preserved_through_deferral(patch_midi: None) -> None:
    ...
    spy0 = conftest.SpyMidiOut()
    seq.midi_out = spy0
    seq.set_device_latency(0, 0)  # deferred
    ...
    await asyncio.sleep(0.05)
    assert [m.control for m in spy0.sent if m.type == "control_change"] == [
        99,
        98,
        6,
        38,
...
async def test_stop_cancels_deferred_and_panic_silences(patch_midi: None) -> None:
    ...
    spy0 = conftest.SpyMidiOut()
    seq.midi_out = spy0
    seq.set_device_latency(0, 0)  # fast → deferred
...
def test_warn_if_high_latency_fires(
    ...
    warnings = [
        ...
        if "latency compensation" in r.getMessage().lower()
    ]
...
def test_warn_if_high_latency_silent_below_threshold(
    patch_midi: None, caplog: pytest.LogCaptureFixture
) -> None:
    caplog.set_level(logging.WARNING, logger="subsequence.composition")
    ...
    comp._warn_if_high_latency()
    assert not [
        r for r in caplog.records if "latency compensation" in r.getMessage().lower()
    ]


def test_render_wires_device_latencies(
    tmp_path: pathlib.Path, patch_midi_multi: typing.Dict[str, conftest.NamedSpyMidiOut]
) -> None:
    ...
    def spy_set(device: typing.Any, latency_ms: float) -> None:
        set_calls.append((device, latency_ms))
        orig_set(device, latency_ms)

    def spy_add(name: str, port: typing.Any, latency_ms: float = 0.0) -> int:
        add_calls.append((name, latency_ms))
        return orig_add(name, port, latency_ms)
