...
async def _send_recv(
    reader: asyncio.StreamReader, writer: asyncio.StreamWriter, code: str
) -> str:
...
async def test_blocked_builtins(composition: subsequence.Composition) -> None:
    ...
    for name in ("help()", "input()", "exit()", "quit()", "breakpoint()"):
        ...
        assert "RuntimeError" in result, (
            f"{name} should raise RuntimeError, got: {result}"
        )
...
@pytest.mark.asyncio
async def test_eval_does_not_block_event_loop(
    ...
    assert fast_duration < 0.3, (
        f"Fast eval took {fast_duration:.2f}s - event loop was blocked"
    )
