...
def make_builder(
    cycle: int = 0, length: float = 4.0
) -> typing.Tuple[
    subsequence.pattern_builder.PatternBuilder, subsequence.pattern.Pattern
]:
...
def test_cellular_automaton_2d_degenerate_grid_is_empty() -> None:
    ...
    assert (
        subsequence.sequence_utils.generate_cellular_automaton_2d(rows=0, cols=16) == []
    )
    assert subsequence.sequence_utils.generate_cellular_automaton_2d(
        rows=3, cols=0
    ) == [[], [], []]
...
def test_slide_out_of_range_note_index_raises_musically() -> None:
    ...
    with pytest.raises(
        ValueError, match="note index 5 is outside this pattern's 4 notes"
    ):
...
def test_quantize_midpoints_all_snap_later() -> None:
    ...
    events = tuple(
        dataclasses.replace(e, beat=b) for e, b in zip(m.events, (0.125, 0.375))
    )
...
def test_wing_discover_returns_none_on_unreachable_network(
