...
def test_contour_factor_pulls_toward_the_target() -> None:
    ...
    state = subsequence.MelodicState(
        key="C", mode="ionian", low=48, high=84, nir_strength=0.0
    )

    highs = [
        state.choose_next(None, random.Random(seed), contour_target=1.0)
        for seed in range(40)
    ]
    state.history.clear()
    lows = [
        state.choose_next(None, random.Random(seed), contour_target=0.0)
        for seed in range(40)
    ]
...
def test_generate_is_deterministic_and_warns_without_seed() -> None:
    ...
    assert M.generate(rhythm=[0, 1, 2, 3], seed=5) == M.generate(
        rhythm=[0, 1, 2, 3], seed=5
    )
...
def test_generate_scale_constrains_candidates() -> None:
    ...
    line = M.generate(
        rhythm=list(range(16)), length=16, scale="minor_pentatonic", seed=4
    )
...
def test_generate_max_pitches_caps_the_pool() -> None:
    ...
    assert (
        len({(e.pitch.step, e.pitch.octave, e.pitch.chroma) for e in line.events}) <= 3
    )
...
def test_generate_ignores_state_rest_probability() -> None:
    ...
    state = subsequence.MelodicState(
        key="C", mode="minor", low=48, high=72, rest_probability=0.9
    )
...
class _StubHarmony:
    ...
    def __init__(
        self, current: typing.Any, following: typing.Any, change_at: float = 4.0
    ) -> None:
...
def _builder(
    harmony: typing.Any = None, length: float = 8.0, seed: int = 1
...
) -> typing.List[typing.Tuple[float, int]]:
    out = []
...
def test_fit_snaps_strong_beats_to_chord_tones() -> None:
    ...
    for beat, pitch in _placed(snapped):
        assert (
            pitch % 12 in chord_pcs
        )  # B (degree 2) pulled to a neighbouring chord tone
...
def test_fit_defaults_split_by_intent() -> None:
    ...
    assert all(
        pitch % 12 == 11 for _, pitch in _placed(hand)
    )  # typed degrees untouched
...
def test_sketch_f_hook_generates_develops_and_places(
