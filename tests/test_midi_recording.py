...
def test_save_recording_creates_valid_midi_file(
    ...
    tempo_events = [
        ...
        if isinstance(m, mido.MetaMessage) and m.type == "set_tempo"
    ]
...
def test_save_recording_delta_ticks_are_correct(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
...
def test_save_recording_skips_when_no_events(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
...
def test_save_recording_skips_when_not_recording(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
    ...
    seq.recorded_events.append(
        (0.0, mido.Message("note_on", channel=0, note=60, velocity=100))
    )
    seq.save_recording()
...
def test_save_recording_generates_timestamp_filename(
    tmp_path: pathlib.Path, patch_midi: None, monkeypatch: pytest.MonkeyPatch
) -> None:
