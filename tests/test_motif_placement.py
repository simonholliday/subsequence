...
def _builder(
    key: str = "A", scale: str = "minor", length: float = 4.0, seed: int = 1, **kwargs
...
def test_degree_octave_and_chroma() -> None:
    ...
    p.motif(
        M.degrees([Degree(5, octave=1), Degree(5, chroma=1)], durations=0.5), root=60
    )
...
def test_harmony_dependent_specs_raise_clearly_without_the_clock() -> None:
    ...
    with pytest.raises(ValueError, match="harmonic clock"):
        p.motif(
            M.from_events(
                [subsequence.MotifEvent(beat=0.0, pitch=Approach(ChordTone("root")))],
                length=1,
            )
        )
    ...
    p.motif(
        M.from_events(
            [subsequence.MotifEvent(beat=0.0, pitch=Approach(Degree(1)))], length=1
        )
    )
...
def test_approach_below_midi_zero_raises_clearly() -> None:
    ...
    with pytest.raises(ValueError, match="outside 0"):
        p.motif(
            M.from_events(
                [subsequence.MotifEvent(beat=0.0, pitch=Approach(0))], length=1
            )
        )
...
def test_chord_tone_outside_window_raises() -> None:
    ...
    with pytest.raises(ValueError, match="window"):
        p.motif(
            M.from_events(
                [subsequence.MotifEvent(beat=2.0, pitch=ChordTone(1))], length=4
            )
        )
