...
def test_quantize_snaps_notes_only () -> None:
	...
	m = M.from_events([MotifEvent(beat=1.07, pitch=60)], length=4, controls=(
...
def test_rhythm_strips_pitches_and_controls () -> None:
	...
	m = (M.degrees([1, 5], velocities=[100, 80]) & M.cc_ramp(74, 0, 127, beat_end=2)).rhythm()
...
def test_phrase_lifts_pitch_transforms_segment_wise () -> None:
	...
	assert P([a, b]).transpose(steps=1).segments == (a.transpose(steps=1), b.transpose(steps=1))
...
def test_describe_smoke () -> None:
	...
	acid = M.hits("kick", beats=[0], length=4).pitched("root") & M.cc_ramp(74, 20, 110, beat_end=4)
