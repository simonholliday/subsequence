...
def test_pattern_device_by_index(patch_midi: None) -> None:
    ...
    comp = subsequence.Composition(bpm=120)
    comp._additional_outputs.append(
        subsequence.composition._AdditionalOutput(device="Secondary MIDI")
    )
...
async def test_panic_all_devices(patch_midi: None) -> None:
    ...
    def has_all_notes_off(spy: conftest.SpyMidiOut) -> bool:
        return any(m.type == "control_change" and m.control == 123 for m in spy.sent)
...
def test_unknown_input_device_name_warns_and_never_matches(
    patch_midi: None, caplog
) -> None:
...
def test_multiple_clock_follow_raises_error(patch_midi: None) -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Only one input device can be configured to follow external clock",
    ):
