...
def test_vary_changes_only_pitches_preserving_rhythm() -> None:
    ...
    assert [event.beat for event in varied.events] == [
        0.0,
        1.0,
        2.0,
        3.0,
    ...
    changed = sum(
        1 for before, after in zip(steps_of(CALL), steps_of(varied)) if before != after
    )
    assert changed == 2
...
def test_vary_degree_floor() -> None:
    ...
    for seed in range(20):
        assert all(
            step >= 1
            for step in steps_of(low.vary(notes=4, position="anywhere", seed=seed))
            if step
        )
...
def test_develop_label_plan() -> None:
    ...
    phrase = subsequence.Phrase.develop(
        CALL, bars=8, plan=["a", "a", "a", "b"], seed=11
    )
...
def test_develop_validates_bars_against_units() -> None:
    ...
    with pytest.raises(ValueError, match="beats"):
        subsequence.Phrase.develop(
            CALL, bars=2, plan=["a", "b"], seed=1
        )  # 4-beat units vs an 8-beat motif
...
def test_reroll_keeps_boundary_pitches() -> None:
    ...
    def bar_events(value: "subsequence.Phrase") -> typing.List[typing.Any]:
        return [
            e
            for e in value.flatten().events
            if 24.0 <= e.beat < 28.0 and e.pitch is not None
        ]
...
def test_phrase_position_walks_with_the_cycle() -> None:
    ...
    third = _builder(cycle=2)
    third.phrase(value, root=60)

    assert [pitch for _, pitch in placed(first)] == [
        pitch for _, pitch in placed(third)
    ]
...
def test_phrase_wraps_across_its_end() -> None:
    ...
    tonic = _builder(cycle=0)
    tonic.phrase(value, root=60)
    assert (
        notes[2][1] == placed(tonic)[0][1]
    )  # beat 2 of the window = the phrase's start
...
def test_phrase_part_follows_sections_and_silences_unbound(
...
def test_sketch_b_motif_to_phrase_to_placement_to_reroll(
    tmp_path: pathlib.Path, patch_midi: None
) -> None:
    ...
    heard: typing.List[
        typing.Tuple[int, typing.Tuple[typing.Tuple[float, int], ...]]
    ] = []
