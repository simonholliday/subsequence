...
def test_named_presets_resolve_as_expected() -> None:
    ...
    assert [
        c.name()
        for c in subsequence.progression("trance_epic").resolve("A", "aeolian").chords
    ] == ["Am", "F", "C", "G"]
    assert [
        c.name() for c in subsequence.progression("pop_axis").resolve("C").chords
    ] == ["C", "G", "Am", "F"]
    assert [
        c.name()
        for c in subsequence.progression("twelve_bar_blues").resolve("E").chords
    ][:4] == ["E7", "E7", "E7", "E7"]
...
def test_sieve_union_builds_scales_and_pools() -> None:
    ...
    major = su.sieve(
        [(12, 0), (12, 2), (12, 4), (12, 5), (12, 7), (12, 9), (12, 11)], hi=12
    )
...
def test_evenness_picks_out_euclidean_rhythms() -> None:
    ...
    assert su.rhythmic_evenness([0, 4, 8, 12], 16) == pytest.approx(
        1.0
    )  # perfectly even
...
def test_preset_default_voices_resolve_against_gm() -> None:
    ...
    for name in subsequence.motifs._WORLD_RHYTHMS:
        ...
        assert voice in gm.GM_DRUM_MAP, (
            f"{name}: default voice {voice!r} is not a GM drum"
        )
    ...
    notes = [
        note.pitch for step in builder._pattern.steps.values() for note in step.notes
    ]
...
def test_elaborate_subdivides_and_preserves_decorations() -> None:
    ...
    assert d1.spans[0].beats == pytest.approx(2.0) and d1.spans[
        1
    ].beats == pytest.approx(2.0)
...
def test_elaborate_determinism_and_seed_warning() -> None:
    ...
    assert [c.name() for c in blues.elaborate(2).chords] == [
        c.name() for c in blues.elaborate(2).chords
    ]

    # Seeded reproducibility at depth 3.
    assert [c.name() for c in blues.elaborate(3, seed=7).chords] == [
        c.name() for c in blues.elaborate(3, seed=7).chords
    ]
    ...
    pitchset = subsequence.progressions.Progression(
        spans=(
            subsequence.progressions.ChordSpan(
                chord=subsequence.progressions.PitchSet([60, 63, 67]), beats=4.0
            ),
        )
    )
