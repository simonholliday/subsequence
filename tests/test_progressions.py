...
def test_roman_secondary_function() -> None:
    ...
    assert (
        secondary.degree == 5
        and secondary.of == 5
        and secondary.quality == "dominant_7th"
    )
...
def test_factory_unknown_preset_raises() -> None:
    ...
    assert [
        c.name()
        for c in subsequence.progressions.progression("trance_epic")
        .resolve("A", "aeolian")
        .chords
    ] == ["Am", "F", "C", "G"]
...
def test_factory_style_generation_is_seeded() -> None:
    ...
    first = subsequence.progressions.progression(
        style="aeolian_minor", key="A", bars=8, seed=7
    )
    second = subsequence.progressions.progression(
        style="aeolian_minor", key="A", bars=8, seed=7
    )
...
def test_is_decorated_true_for_each_decoration_kind() -> None:
    ...
    assert (
        subsequence.progressions.ChordSpan(**bare, extensions=(7,)).is_decorated is True
    )
    ...
    assert (
        subsequence.progressions.ChordSpan(**bare, spread="open").is_decorated is True
    )
...
def test_with_rhythm_reshapes_spans() -> None:
    ...
    value = subsequence.progressions.progression(["Am", "F", "C", "G"]).with_rhythm(
        [4, 4, 2, 6]
    )
...
def test_concrete_note_name_slash_bass_voices_without_manual_resolve() -> None:
    ...
    realized = subsequence.progressions.realize(
        concrete, 4.0, "C", 4.0, random.Random(0)
    )
...
def test_realize_style_is_deterministic_under_a_seed() -> None:
    ...
    first = subsequence.progressions.realize(
        "phrygian_minor", WHOLE, "C", 32.0, random.Random(7)
    )
    second = subsequence.progressions.realize(
        "phrygian_minor", WHOLE, "C", 32.0, random.Random(7)
    )
...
def test_realize_style_needs_key() -> None:
    ...
    with pytest.raises(ValueError, match="key"):
        subsequence.progressions.realize(
            "phrygian_minor", WHOLE, None, 16.0, random.Random(0)
        )
...
def test_realize_relative_elements_need_key() -> None:
    ...
    value = subsequence.progressions.realize(
        [1, 6], WHOLE, "A", 8.0, random.Random(0), scale="minor"
    )
...
def test_realize_between_rhythm_is_bounded() -> None:
    ...
    value = subsequence.progressions.realize(
        "phrygian_minor", spec, "C", 64.0, random.Random(2)
    )
...
def test_generate_is_key_invariant() -> None:
    ...
    in_c = subsequence.progressions.Progression.generate(
        style="aeolian_minor", bars=8, seed=11, key="C", scale="minor"
    )
    in_g = subsequence.progressions.Progression.generate(
        style="aeolian_minor", bars=8, seed=11, key="G", scale="minor"
    )
...
def test_generate_relative_resolution_matches_concrete_generation() -> None:
    ...
    relative = subsequence.progressions.Progression.generate(
        style="aeolian_minor", bars=8, seed=3
    )
    concrete = subsequence.progressions.Progression.generate(
        style="aeolian_minor", bars=8, seed=3, key="A"
    )
...
def test_generate_without_seed_warns() -> None:
    ...
    with pytest.warns(UserWarning, match="seed"):
        subsequence.progressions.Progression.generate(
            style="functional_major", bars=2, key="C"
        )
...
def test_factory_forwards_hybrid_constraints() -> None:
    ...
    assert (
        via_factory.resolve("A", "minor").chords
        == via_generate.resolve("A", "minor").chords
    )
