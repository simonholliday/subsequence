...
def test_resolve_length_uses_beats_per_bar () -> None:
	assert subsequence.Composition._resolve_length(None, 2, None, None, beats_per_bar=3)[0] == 6.0
	assert subsequence.Composition._resolve_length(None, 2, None, None, beats_per_bar=4)[0] == 8.0
	assert subsequence.Composition._resolve_length(None, 2, None, None)[0] == 8.0	# default 4/4


def test_set_target_bpm_respects_time_signature (patch_midi: None) -> None:
	seq = subsequence.sequencer.Sequencer(output_device_name="Dummy MIDI", initial_bpm=120, time_signature=(3, 4))
...
def test_cellular_1d_memoised_matches_from_scratch () -> None:
	...
	for gen in range(8):
		incremental = su.generate_cellular_automaton_1d(16, rule=30, generation=gen, seed=1)
...
def test_cellular_2d_memoised_matches_from_scratch () -> None:
	...
	fresh = su.generate_cellular_automaton_2d(rows=4, cols=8, rule="B3/S23", generation=6, seed=42)

	su._ca_2d_cache.clear()
	incremental: typing.List[typing.List[int]] = []
	for gen in range(7):
		incremental = su.generate_cellular_automaton_2d(rows=4, cols=8, rule="B3/S23", generation=gen, seed=42)
...
def _reachable_from_tonic (graph: typing.Any, tonic: typing.Any) -> typing.Set[typing.Any]:
...
def test_chord_graph_orphans_now_reachable () -> None:
	...
	cases = [
		(subsequence.chord_graphs.functional_major.DiatonicMajor().build("C"),
			[chord(root_pc=4, quality="minor"), chord(root_pc=11, quality="diminished")]),			# iii, vii°
		(subsequence.chord_graphs.aeolian_minor.AeolianMinor().build("C"),
			[chord(root_pc=3, quality="major"), chord(root_pc=2, quality="diminished"), chord(root_pc=11, quality="diminished")]),	# bIII, ii°, vii°
		(subsequence.chord_graphs.dorian_minor.DorianMinor().build("C"),
			[chord(root_pc=9, quality="diminished")]),													# vi°
		(subsequence.chord_graphs.lydian_major.LydianMajor().build("C"),
			[chord(root_pc=6, quality="diminished"), chord(root_pc=11, quality="minor")]),			# #iv°, vii
	]

	for (graph, tonic), expected in cases:
		reachable = _reachable_from_tonic(graph, tonic)
		for orphan in expected:
			assert orphan in reachable, f"{orphan} should now be reachable from the tonic"
