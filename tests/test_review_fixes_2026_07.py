...
def test_accidental_degree_resolves_under_five_degree_scale () -> None:
	...
	chord = subsequence.progressions.RomanChord(degree=7, accidental=-1, quality="major").resolve(0, "hirajoshi")
...
def _phrase_events (phrase: subsequence.Phrase) -> typing.List[typing.Tuple[typing.Any, ...]]:

	"""Flatten a phrase to comparable (beat, pitch, velocity) tuples."""

	return [(e.beat, e.pitch, e.velocity) for segment in phrase.segments for e in segment.events]
...
def test_sentence_and_reroll_share_the_salt_fix () -> None:
	...
	assert _phrase_events(base.reroll(bar=2, seed=5)) == _phrase_events(base.reroll(bar=2, seed=5))
...
def test_generate_midi_pool_cadence_raises () -> None:
	...
	with pytest.raises(ValueError, match="MIDI pool"):
		subsequence.Motif.generate(rhythm=[0, 1, 2, 3], scale=_MIDI_POOL, seed=1, cadence="strong")
...
def test_generate_midi_pool_degree_pin_raises () -> None:
	...
	with pytest.raises(ValueError, match="MIDI pool"):
		subsequence.Motif.generate(rhythm=[0, 1, 2, 3], scale=_MIDI_POOL, seed=1, pins={-1: subsequence.Degree(5)})
...
def test_generate_midi_pool_int_pin_is_exact_note () -> None:
	...
	generated = subsequence.Motif.generate(rhythm=[0, 1, 2, 3], scale=_MIDI_POOL, seed=1, pins={-1: 63})
...
def test_injected_chord_reads_do_not_advance_voice_leading () -> None:
	...
	c_chord = subsequence.composition._InjectedChord(subsequence.parse_chord("C"), state)
	assert c_chord.tones(60) == [60, 64, 67]

	primed = list(state.previous_voicing or [])

	f_chord = subsequence.composition._InjectedChord(subsequence.parse_chord("F"), state)
...
_AGR_ASSET = pathlib.Path(__file__).parent.parent / "examples" / "assets" / "Swing 16ths 57.agr"
...
def _write_agr_clip (path: pathlib.Path, notes: typing.List[typing.Tuple[float, int]], clip_length: float) -> None:
...
def test_from_agr_chord_clip_raises_grid_error (tmp_path: pathlib.Path) -> None:
	...
	_write_agr_clip(agr, [(0.0, 127), (0.25, 100), (0.25, 90), (0.75, 80)], clip_length=1.0)
...
def test_from_agr_rest_imports_cleanly_with_explicit_grid (tmp_path: pathlib.Path) -> None:
...
async def test_named_drone_mirrors_use_their_own_drum_maps (patch_midi: None) -> None:
	...
	pattern = subsequence.pattern.Pattern(
		channel=0, length=4, device=0,
		mirrors=[(1, 5, {"kick": 50}), (2, 6, {"snare": 40})],
	)
	pattern.raw_note_events.append(subsequence.pattern.RawNoteEvent(
		pulse=0, message_type="note_on", pitch=36, velocity=100, origin="kick",
	))
	...
	assert sorted((e.device, e.channel, e.note) for e in note_ons) == [(0, 0, 36), (1, 5, 50)]
...
async def test_onset_bend_priority_beats_note_push_order (patch_midi: None) -> None:
	...
	pattern.cc_events.append(subsequence.pattern.CcEvent(
		pulse=0, message_type="pitchwheel", value=1234, priority=-1,
	))
...
@pytest.mark.asyncio
async def test_stop_pattern_notes_defers_note_off_on_fast_device (patch_midi: None) -> None:
...
def test_start_harmonic_clock_is_idempotent (patch_midi: None, monkeypatch: pytest.MonkeyPatch) -> None:
