...
async def test_reschedule_event_precedes_pattern_rebuild(patch_midi: None) -> None:
    ...
    def on_reschedule_pulse(
        pulse: int, patterns: list[subsequence.pattern.Pattern]
    ) -> None:
    ...
    reschedule_pulse = (
        pattern.length * sequencer.pulses_per_beat
        - pattern.reschedule_lookahead * sequencer.pulses_per_beat
    )
...
async def test_repeating_callback_fires_without_patterns(patch_midi: None) -> None:
    ...
    await sequencer.schedule_callback_repeating(
        on_tick, interval_beats=2, start_pulse=0, reschedule_lookahead=1
    )
...
async def test_dynamic_length_change_on_reschedule(patch_midi: None) -> None:
    ...
    expected_next_reschedule = int(2 * sequencer.pulses_per_beat) + int(
        (4 - 1) * sequencer.pulses_per_beat
    )
...
async def test_schedule_pattern_includes_cc_events(patch_midi: None) -> None:
    ...
    cc_events = [
        e
        for e in sequencer.event_queue
        if e.message_type in ("control_change", "pitchwheel")
    ]
...
async def test_osc_event_dispatched_to_server(patch_midi: None) -> None:
    ...
    heapq.heappush(
        ...
        subsequence.sequencer.MidiEvent(
            pulse=0, message_type="osc", channel=0, data=("/fader/1", (0.75,))
        ),
    )
...
async def test_osc_event_noop_without_server(patch_midi: None) -> None:
    ...
    heapq.heappush(
        sequencer.event_queue,
        subsequence.sequencer.MidiEvent(
            pulse=0, message_type="osc", channel=0, data=("/fader/1", (0.5,))
        ),
    )
...
def test_event_counter_resets_independently_per_sequencer() -> None:
    ...
    seq_a = subsequence.sequencer.Sequencer(
        output_device_name="Dummy MIDI", initial_bpm=120
    )
    seq_b = subsequence.sequencer.Sequencer(
        output_device_name="Dummy MIDI", initial_bpm=120
    )

    event_a = subsequence.sequencer.MidiEvent(
        pulse=0, message_type="control_change", channel=0, control=1, value=0
    )
    event_b = subsequence.sequencer.MidiEvent(
        pulse=0, message_type="control_change", channel=0, control=2, value=0
    )
