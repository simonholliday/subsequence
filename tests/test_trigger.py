...
async def test_trigger_with_quantize_beat_boundary(patch_midi: None) -> None:
    ...
    assert all(p >= 24 for p in event_pulses), (
        f"Expected events at pulse >= 24, got {event_pulses}"
    )
...
async def test_trigger_with_quantize_bar_boundary(patch_midi: None) -> None:
    ...
    event_pulses = [e.pulse for e in composition._sequencer.event_queue]
    assert all(p >= 96 for p in event_pulses), (
        f"Expected events at pulse >= 96, got {event_pulses}"
    )
...
async def test_trigger_with_drum_note_map(patch_midi: None) -> None:
    ...
    composition.trigger(
        builder, channel=9, drum_note_map=gm_drums.GM_DRUM_MAP, quantize=0
    )
...
async def test_trigger_uses_builder_chaining(patch_midi: None) -> None:
    ...
    def builder(p: "subsequence.pattern_builder.PatternBuilder") -> None:
        # Use method chaining
        p.note(60, beat=0, velocity=100, duration=0.5).note(
            62, beat=0.5, velocity=90, duration=0.5
        )
    ...
    assert note_on_count >= 2, (
        f"Expected at least 2 note_on events, got {note_on_count}"
    )
