...
def _running_pattern_stub (
	...
	pat = subsequence.pattern.Pattern(channel=channel, length=4, device=device, mirrors=mirrors)
	pat._muted = False
	pat._tweaks = {}
	pat._cycle_count = 0
	return pat
...
async def test_unregister_stops_active_notes_on_mirrors (patch_midi: None) -> None:
	...
	pattern = subsequence.pattern.Pattern(channel=2, length=4, device=0, mirrors=[(1, 7)])
