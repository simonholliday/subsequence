...
def test_choose_next_is_unaffected_by_labels() -> None:
    ...
    labelled: subsequence.weighted_graph.WeightedGraph = (
        subsequence.weighted_graph.WeightedGraph()
    )
    bare: subsequence.weighted_graph.WeightedGraph = (
        subsequence.weighted_graph.WeightedGraph()
    )
    ...
    assert labelled.choose_next("a", random.Random(5)) == bare.choose_next(
        "a", random.Random(5)
    )
